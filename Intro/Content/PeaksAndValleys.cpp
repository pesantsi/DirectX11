...
PeaksAndValleys::PeaksAndValleys(const std::shared_ptr<DX::DeviceResources>& deviceResources) : 
	mNumRows(0),
	mNumCols(0),
	mNumVertices(0),
	mNumFaces(0)
...
void PeaksAndValleys::CreateDeviceDependentResources()
{
	DWORD m = 129;
	DWORD n = 129;
	float dx = 1.0f;
...
void PeaksAndValleys::ReleaseDeviceDependentResources()
{
	m_VB.Reset();
	m_IB.Reset();
}

void PeaksAndValleys::Update(DX::StepTimer const& timer)
{
